...
#define BUFFER_NUM 9
#define T_SH_REG_NUM 4

SC_MODULE(ACCUMULATOR_THIRD) {

/* Input Ports  */
    sc_in<sc_int<32> > input_T[BUFFER_NUM]; //Input from tree adder
    sc_in<sc_int<32> > input_S[2]; //Input from Boundary SRAM
...
/* Shift Registers  */
    sc_signal<sc_int<32> > shift_regs[BUFFER_NUM][T_SH_REG_NUM];

/* Output Ports   */
    sc_out<sc_int<32> > output_P[BUFFER_NUM-2]; // output to post process
    sc_out<sc_int<32> > output_S[2]; // output to boundary SRAM
